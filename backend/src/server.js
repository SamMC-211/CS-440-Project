...
app.post("/api/login", (req, res) => {
  ...
  setTimeout(() => {
    ...
    if (!username || !password)
      return res.status(400).json({ ok: false, message: "Missing" }); // If username or password not recieved, respond accordingly

    const user = USERS.find(
      (u) => u.username === username && u.password === password
    ); // returns first element in USERS array where POSTed username/password match
    if (!user)
      return res
        .status(401)
        .json({ ok: false, message: "Invalid credentials" }); //If no user is returned, respond accordingly
