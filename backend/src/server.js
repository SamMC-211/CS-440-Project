    ...
    db.get(
      ...
      (err, row) => {
        if (err) {
          return res
            .status(500)
            .json({ ok: false, message: "Login Error", error: err.message });
        }
...
app.get("/api/data", (req, res) => {
  db.all("SELECT * FROM users ORDER BY lastname ASC LIMIT 10", (err, rows) => {
    if (err) {
      return res
        .status(500)
        .json({ error: err.message, message: "Fetch Users Failed" });
    } else if (rows) {
